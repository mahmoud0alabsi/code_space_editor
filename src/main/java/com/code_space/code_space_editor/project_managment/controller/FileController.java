...
import com.code_space.code_space_editor.project_managment.dto.file.CreateFileVersionDTO;
...
public class FileController {
    ...
    public ResponseEntity<File> createFile(
        ...
        File file = fileService.createFile(projectId, branchId, fileDTO.getName(), fileDTO.getLanguage(),
    ...
    @PreAuthorize("@permissionService.hasFilePermission(#projectId, #branchId, #fileId, 'COLLABORATOR')")
    public ResponseEntity<FileVersion> creatFileVersion(
            @PathVariable Long projectId,
            @PathVariable Long branchId,
            @PathVariable Long fileId,
            @RequestBody CreateFileVersionDTO newVersion) {
        FileVersion fileVersion = fileService.createFileVersion(fileId, newVersion.getContent(),
    ...
    @PreAuthorize("@permissionService.hasFilePermission(#projectId, #branchId, #fileId, 'VIEWER')")
    ...
    public ResponseEntity<String> getSpecificFileVersionContent(
        ...
        String content = fileService.getSpecificVersionContent(fileId, versionNumber);
        return ResponseEntity.ok()
                .contentType(MediaType.TEXT_PLAIN)
                .body(content);
    }

    private void validatePathVariables(Long projectId, Long branchId, Long fileId) {
